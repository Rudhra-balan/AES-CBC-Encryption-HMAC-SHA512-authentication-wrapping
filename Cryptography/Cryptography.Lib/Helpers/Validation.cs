...
public partial class Helpers
{
    ...
    /// <param name="IV"></param>
    public void CoreValidation(byte[] data, byte[] key, byte[] IV)
    {
        //Checks the validation of the supplied arguments
        if (data == null || data.Length <= 0) throw new ArgumentNullException("No data has been supplied");
        if (key == null || key.Length <= 0) throw new ArgumentNullException("No key has been supplied");
        if (IV == null || IV.Length <= 0) throw new ArgumentNullException("No IV has been supplied");
    }
    ...
    /// <param name="data"></param>
    /// <returns></returns>
    public byte[] EncryptedDataValidation(object data)
    {
        ...
        if (data != null)
        {
            ...
            else
            {
                throw new InvalidCastException(
                    "The file format for the encrypted data can only be a string or byte[].");
            }
        }
        else
        {
    ...
    /// <param name="key"></param>
    /// <returns></returns>
    public byte[] KeyValidation(object key)
    {
        byte[] theKey = null;
        if (key != null)
        {
    ...
    /// <param name="IV"></param>
    /// <returns></returns>
    public byte[] IVValidaton(object IV)
    {
        byte[] theIV = null;
        if (IV != null)
        {
